$(document).ready(function() {
  ...
  if ($('.article').length) {
    $('.article.container figure img').each(function(index, element) {
      $(element).attr('src', $(element).attr('src').replace('../', '../../'));
    });
  ...
  if ($('.home').length) {
    if (width >= 1200) {
      ...
      setInterval(function() {
        if (parseInt($('.holder').css('margin-left')) <= -($('.holder figure').length - 4) * (width / 3)) 
          $('.holder').animate({
            'margin-left': '0px'
          }, 1200, 'easeOutCubic');
        else 
      ...
      setInterval(function() {
        if (parseInt($('.holder').css('margin-left')) <= -($('.holder figure').length - 3) * (width / 2)) 
          $('.holder').animate({
            'margin-left': '0px'
          }, 1200, 'easeOutCubic');
        else 
      ...
      setInterval(function() {
        if (parseInt($('.holder').css('margin-left')) <= -($('.holder figure').length - 1) * width) 
          $('.holder').animate({
            'margin-left': '0px'
          }, 1200, 'easeOutCubic');
        else 
    ...
    $('.overlay .overlay-logo').mouseenter(function() {
    ...
    $('.overlay').click(function() {
      $('.overlay .overlay-logo').unbind('mouseenter mouseleave');
      $('.overlay').stop().animate({
        opacity: 0
      }, 600, function() {
        $(this).remove();
  ...
  if ($('.contact').length) {
    ...
    var instaFeed = new Instafeed({
      ...
      template: '<div class="col-lg-2 col-md-4 col-sm-4 col-xs-6 nopadding"><figure><img src="{{image}}" /></figure></div>'
    }).run();
    ...
    $('#contact').validator().on('submit', function(e) {
      if (e.isDefaultPrevented()) {
        ...
        e.preventDefault();
        $('#contact').off('submit');
        $('#contact #submit').html('<img src="static/images/loading.gif" />').addClass('disabled');
        $.post('../static/scripts/contact.php', $(this).serialize(), function(result) {
          $('#contact').hide(300);
          $('.contact .alert-success').css('display', 'block');
        }).fail(function() {
          $('#contact').hide(300);
          $('.contact .alert-danger').css('display', 'block');
        });
      }
    });
  }
  ...
  if ($('._portfolio').length) {
    ...
    var slideRight = function() {
      $('._portfolio .slideshow-holder').find('.fa-chevron-right').click()
    };
    var slideTimer = setInterval(function() {
      slideRight()
    }, 6000);
    $('._portfolio .slideshow-holder').find('.fa-chevron-right').click(function() {
      if (parseInt($('._portfolio .slideshow').css('margin-left')) <= -($('._portfolio .slideshow').find('figure').length - 1) * $('.slideshow-holder').outerWidth()) 
        $('._portfolio .slideshow').animate({
          'margin-left': '0px'
        }, 1200, 'easeOutCubic');
      else 
        $('._portfolio .slideshow').animate({
          'margin-left': '-=' + $('.slideshow-holder').outerWidth() + 'px'
        }, 600, 'easeOutCubic');
    ...
    $('._portfolio .slideshow-holder').find('.fa-chevron-left').click(function() {
      if ($('._portfolio .slideshow').css('margin-left') >= '0px') 
        $('._portfolio .slideshow').animate({
          'margin-left': '-' + (
          $('._portfolio .landing-image').find('figure').length - 1) * $('.slideshow-holder').outerWidth() + 'px'
        }, 1200, 'easeOutCubic');
      else 
        $('._portfolio .slideshow').animate({
          'margin-left': '+=' + $('.slideshow-holder').outerWidth() + 'px'
  ...
  if ($('.project').length) {
    ...
    var slideRight = function() {
      $('.project .project-slide').find('.fa-chevron-right').click()
    };
    var slideTimer = setInterval(function() {
      slideRight()
    }, 6000);
    $('.project .project-slide').find('.fa-chevron-right').click(function() {
      if (parseInt($('.project .holder').css('margin-left')) <= -($('.project .holder').find('figure').length - 1) * $('.project .project-slide').outerWidth()) 
        $('.project .holder').animate({
          'margin-left': '0px'
        }, 1200, 'easeOutCubic');
      else 
        $('.project .holder').animate({
          'margin-left': '-=' + $('.project .project-slide').outerWidth() + 'px'
        }, 600, 'easeOutCubic');
      clearInterval(slideTimer);
      slideTimer = setInterval(function() {
        slideRight()
    ...
    $('.project .project-slide').find('.fa-chevron-left').click(function() {
      if ($('.project .holder').css('margin-left') >= '0px') 
        $('.project .holder').animate({
          'margin-left': '-' + (
          $('.project .holder').find('figure').length - 1) * $('.project .project-slide').outerWidth() + 'px'
        }, 1200, 'easeOutCubic');
      else 
        $('.project .holder').animate({
          'margin-left': '+=' + $('.project .project-slide').outerWidth() + 'px'
...
!function(r) {
  function t(t) {
    ...
    return t && t.constructor == Array && 3 == t.length
      ? t
      : (a = /rgb\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*\)/.exec(t))
        ? [
          parseInt(a[1]),
          parseInt(a[2]),
          parseInt(a[3])
        ]
        : (a = /rgb\(\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*\)/.exec(t))
          ? [
            2.55 * parseFloat(a[1]),
            2.55 * parseFloat(a[2]),
            2.55 * parseFloat(a[3])
          ]
          : (a = /#([a-fA-F0-9]{2})([a-fA-F0-9]{2})([a-fA-F0-9]{2})/.exec(t))
            ? [
              parseInt(a[1], 16),
              parseInt(a[2], 16),
              parseInt(a[3], 16)
            ]
            : (a = /#([a-fA-F0-9])([a-fA-F0-9])([a-fA-F0-9])/.exec(t))
              ? [
                parseInt(a[1] + a[1], 16),
                parseInt(a[2] + a[2], 16),
                parseInt(a[3] + a[3], 16)
              ]
              : (a = /rgba\(0, 0, 0, 0\)/.exec(t))
                ? e.transparent
                : e[r.trim(t).toLowerCase()]
  }
  function a(a, e) {
    var o;
    do {
      if (o = r.css(a, e), "" != o && "transparent" != o || r.nodeName(a, "body")) 
        break;
      e = "backgroundColor"
  ...
  r.each([
    ...
    r.fx.step[o] = function(r) {
      r.colorInit || (r.start = a(r.elem, o), r.end = t(r.end), r.colorInit = !0),
      r.elem.style[o] = "rgb(" + [
        Math.max(Math.min(parseInt(r.pos * (r.end[0] - r.start[0]) + r.start[0]), 255), 0),
        Math.max(Math.min(parseInt(r.pos * (r.end[1] - r.start[1]) + r.start[1]), 255), 0),
        Math.max(Math.min(parseInt(r.pos * (r.end[2] - r.start[2]) + r.start[2]), 255), 0)
      ].join(",") + ")"
    }
  });
  var e = {
...
(function() {
  ...
  e = function() {
        ...
        if (this.options.template != null && typeof this.options.template == "string") {
          ...
          for (y = 0, E = a.length; y < E; y++) 
            ...
            this.options.links === !0
              ? (t = document.createElement("a"), t.href = s.link, t.appendChild(f), n.appendChild(t))
              : n.appendChild(f);
    ...
    e.prototype._makeTemplate = function(e, t) {
      ...
      while (r.test(n)) 
        i = n.match(r)[1],
        s = (o = this._getObjectProperty(t, i)) != null
          ? o
          : "",
        n = n.replace(r, "" + s);
      return n
